...
public class UniformCostSearch {
	...
	private Boolean checkGoal(int[][] state) {
		int queens = 0;
		for(int i = 0 ; i < 8 ; i++) {
			for(int j = 0 ; j < 8 ; j++) {
				if(state[i][j] == 8) {
	...
	public void solve() {
		...
		for(int i = 0 ; i < actions.size() ; i++) {
			...
			Node child = new Node(newState, n, actions.get(i), 2);
			frontier.add(child);
		}
		while(true) {
			n = frontier.pop();
			...
			for(int i = 0 ; i < actions.size() ; i++) {
				...
				Node c = frontier.nodeContains(newState);
				if(c == null && !explored.contains(newState)) {
					Node child = new Node(newState, n, currentAction, n.cost+1);
					frontier.add(child);
				}
				else if(c != null && c.cost > n.cost+1) {
					c.parent = n;
...
//				{ 0, 0, 0, 0, 0, 0, 0, 0 }
//		}; 
//		UniformCostSearch p = new UniformCostSearch(board);
//		p.solve();
//	}
//}